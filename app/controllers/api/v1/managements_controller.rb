      ...
      if request.get?
        ...
        if params[:parent_id] && params[:area] && params[:grade]
          ...
          if existent.nil?
            ...
            render json: {
              message: "El área '#{new_management.area}' ha sido creada exitosamente.",
              status: 200
            }
          else
            render json: {
              message: "El área '#{existent.area}' ya existe.",
              status: 404
            }
          end
        else
          render json: {
            message: 'Faltan parámetros para creación de nuevo recurso.',
            status: 404
          }
        end
      end
      ...
      if params[:id] && params[:area] && params[:grade]
        ...
        unless updated_management.nil?
          ...
          if updated_management.save
            ...
            # Grade under 0
            render json: {
              message: 'Ocurrió un error inesperado, reintente nuevamente.',
              status: 404
            }
          end
        end
        ...
        render json: {
          message: 'Faltan parámetros para update de recurso.',
          status: 404
        }
      end
    end
